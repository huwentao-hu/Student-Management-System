...
flowchart LR
    ...
    Browser[React 单页应用] --> Nginx[EC2 Nginx HTTPS]
    Nginx --> Auth[Spring Boot Docker 认证拦截器]
    Auth --> Controller
    Auth --> TokenRepo[令牌仓储]
...
- 前端托管目标：Cloudflare Pages 静态站点
- 生产后端目标：EC2 上的 Spring Boot Docker 容器，由 Nginx 提供 HTTPS
- 生产数据库目标：禁止公网访问的 RDS PostgreSQL
- 自动部署目标：GitHub Actions 使用 OIDC 推送 ECR，并通过 SSM 部署 EC2
...
- `.github/workflows/deploy-backend.yml`：后端测试、ECR 推送和 EC2 自动部署
...
### ADR-025：生产后端采用 EC2 Docker 与私有 RDS

生产后端运行在 EC2 的 Docker 容器中，容器仅绑定回环地址，由 Nginx 对外提供 HTTPS。PostgreSQL 使用不允许公网访问的 RDS，并只允许 EC2 安全组访问。GitHub Actions 使用 OIDC 临时身份推送 ECR，再通过 SSM 部署，避免长期 AWS 密钥和长期开放 SSH。
...
- 前端计划部署到 Cloudflare Pages；Spring Boot 使用 EC2 Docker，PostgreSQL 使用私有 RDS。
- 生产后端已部署到 EC2，公网 API 为 `https://api.751905.xyz`。
- GitHub Actions 已实现后端测试、ECR 镜像推送、SSM 部署与健康检查。
